      ...
      if (req.status == 200) {
        ...
        if (response) {
          ...
          Pebble.sendAppMessage({
			...
			"hidetemp" : (options["hidetemp"] == "true" ? 1 : 0),
			"hidewicon" : (options["hidewicon"] == "true" ? 1 : 0),
			"min" : (options["min"] == "true" ? 1 : 0),
			"hidesep" : (options["hidesep"] == "true" ? 1 : 0),
...
var locationOptions = { "timeout": 30000, "maximumAge": 2000000 };
...
Pebble.addEventListener('showConfiguration', function(e) {
  var uri = 'http://www.themapman.com/pebblewatch/90deg0_4.html?' +
	...
	'&hidewicon=' + encodeURIComponent(options['hidewicon']) +
	'&min=' + encodeURIComponent(options['min']) +
	'&hidesep=' + encodeURIComponent(options['hidesep']) +
